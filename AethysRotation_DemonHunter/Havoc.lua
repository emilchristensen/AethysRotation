...
-- APL from DemonHunter_Havoc_T20M on 08/15/2017
...
local Settings = {
  ...
  DemonHunter = AR.GUISettings.APL.DemonHunter
}
...
-- Metamorphosis Adjusted Cooldown
...
local function WaitingForNemesis()
  ...
  return not (not S.Nemesis:IsAvailable() or S.Nemesis:IsReady() or NemCooldownRemains > Target:TimeToDie() or NemCooldownRemains > 60)
end
...
local function WaitingForChaosBlades()
  ...
  return not (not S.ChaosBlades:IsAvailable() or S.ChaosBlades:IsReady() or CBCooldownRemains > Target:TimeToDie() or CBCooldownRemains > 60)
end
...
local function PoolingForMeta()
  return AR.CDsON() and not S.Demonic:IsAvailable() and S.Metamorphosis:CooldownRemains() < 6 and Player:FuryDeficit() > 30 and (not WaitingForNemesis() or S.Nemesis:CooldownRemains() < 10) and (not WaitingForChaosBlades() or S.ChaosBlades.CooldownRemains() < 6)
end
...
local function PoolingForChaosStrike()
  return AR.AoEON() and S.ChaosCleave:IsAvailable() and Player:FuryDeficit() > 40
...
local function APL()
  ...
  if not Player:AffectingCombat() then
    ...
    if Target:Exists() and Player:CanAttack(Target) and not Target:IsDeadOrGhost() then
      if S.Metamorphosis:IsCastable() and (AR.CDsON() and not (S.DemonReborn:IsAvailable() and S.Demonic:IsAvailable())) then
  ...
  if Target:Exists() and Player:CanAttack(Target) and not Target:IsDeadOrGhost() then
    ...
    if AR.CDsON() then
      ...
      if S.Metamorphosis:IsCastable() and S.Demonic:IsAvailable() and Player:Buff(S.Metamorphosis) and Player:Fury() < 40 then
        if AR.Cast(S.Metamorphosis) then return "Cast Metamorphosis"; end
      end
      ...
      if S.Nemesis:IsCastable() and (Player:Buff(S.ChaosBlades) or Player:Buff(S.Metamorphosis) or MetamorphosisAdjustedCooldown() < 20 or Target:TimeToDie() <= 60) then
      ...
      if S.ChaosBlades:IsCastable() and (Player.Buff(S.Metamorphosis) or MetamorphosisAdjustedCooldown() > 60 or Target:TimeToDie() <= 12) then
    ...
    if S.Demonic:IsAvailable() then
      ...
      -- return since we are in run_action_list
      return false
    end
    ...
    if S.FelRush:IsCastable() and (S.Momentum:IsAvailable() or S.FelMastery:IsAvailable()) and (not S.Momentum:IsAvailable() or (S.FelRush:Charges() == 2 or S.VengefulRetreat:CooldownRemains() > 4) and not Player:Buff(S.Momentum)) and (not S.FelMastery:IsAvailable() or Player:FuryDeficit() >= 25) and S.FelRush:Charges() == 2 then
    ...
    if AR.AoEON() and S.FelBarrage:IsCastable() and ((Player:Buff(S.Momentum) or not S.Momentum:IsAvailable()) and Cache.EnemiesCount[30] > DesiredTargets) then
    ...
    if S.FelBlade:IsCastable() and Target:IsInRange(15) and Player:Fury() < 15 and S.BladeDance:CooldownRemains() < 2 * Player:GCD() then
      if AR.Cast(S.FelBlade) then return "Cast FelBlade"; end
    end

    -- actions.normal+=/death_sweep,if=variable.blade_dance
    if S.DeathSweep:IsReady() and BladeDance() and Player:Buff(S.Metamorphosis) then
    ...
    if S.FelRush:IsCastable() and (not S.Momentum:IsAvailable() and not S.FelMastery:IsAvailable()) and S.FelRush:Charges() == 2 then
      if AR.Cast(S.FelRush) then return "Cast FelRush"; end
    end

    -- actions.normal+=/fel_eruption
    if S.FelEruption:IsReady() and S.FelEruption:IsAvailable() then
    ...
    if S.FuryoftheIllidari:IsCastable() and (Cache.EnemiesCount[6] > DesiredTargets) or ((not S.Momentum:IsAvailable() or Player:Buff(S.Momentum)) and (not S.ChaosBlades:IsAvailable() or Player:Buff(S.ChaosBlades) or S.ChaosBlades:CooldownRemains() > 30 or Target:TimeToDie() < S.ChaosBlades:CooldownRemains())) then 
    ...
    if S.BladeDance:IsReady() and BladeDance() and not S.Metamorphosis:IsReady() then
    ...
    if S.ThrowGlaive:IsCastable() and (S.Bloodlet:IsAvailable() and Cache.EnemiesCount[30] >= 3 and (not S.MasteroftheGlaive.IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.Momentum))) then
      if AR.Cast(S.ThrowGlaive) then return "Cast ThrowGlaive"; end
    end

    -- actions.normal+=/felblade,if=fury.deficit>=30+buff.prepared.up*8
    if S.FelBlade:IsCastable() and Target:IsInRange(15) and Player:FuryDeficit() >= (30 + Player:Buff(S.Prepared) and 8 or 0) then
      if AR.Cast(S.FelBlade) then return "Cast FelBlade"; end
    end
    ...
    if S.EyeBeam:IsReady() and (Cache.EnemiesCount[12] > DesiredTargets or (S.BlindFury:IsAvailable() and Player:FuryDeficit() >= 35)) then
    ...
    if S.Annihilation:IsCastable() and ( (S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.Momentum) or Player:FuryDeficit() < 30 + (Player:Buff(S.Prepared) and 8 or 0) or Player:BuffRemains(S.Metamorphosis) < 5) and not PoolingForBladeDance()) then
    ...
    if S.ThrowGlaive:IsCastable() and (S.Bloodlet:IsAvailable() and (not S.MasteroftheGlaive:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.Momentum))) then
      if AR.Cast(S.ThrowGlaive) then return "Cast ThrowGlaive"; end
    end

    -- actions.normal+=/throw_glaive,if=!talent.bloodlet.enabled&buff.metamorphosis.down&spell_targets>=3
    if S.ThrowGlaive:IsCastable() and (not S.Bloodlet:IsAvailable() and not Player:Buff(S.Metamorphosis) and Cache.EnemiesCount[30] >= 3) then
      if AR.Cast(S.ThrowGlaive) then return "Cast ThrowGlaive"; end
    end
    ...
    if S.ChaosStrike:IsCastable() and ((S.DemonBlades:IsAvailable() or not S.Momentum:IsAvailable() or Player:Buff(S.Momentum) or Player:FuryDeficit() < 30 + (Player:Buff(S.Prepared) and 8 or 0)) and not PoolingForChaosStrike() and not PoolingForMeta() and not PoolingForBladeDance()) then
    ...
    if S.FelRush:IsCastable() and (not S.Momentum:IsAvailable() and (S.DemonBlades:IsAvailable() or not Player:Buff(S.Metamorphosis))) then
      if AR.Cast(S.FelRush) then return "Cast FelRush"; end
    end
    ...
    if S.ThrowGlaive:IsCastable() and (not Target:IsInRange(5) or Target:IsInRange(30)) then
      if AR.Cast(S.ThrowGlaive) then return "Cast ThrowGlaive"; end
    end

    -- actions.normal+=/felblade,if=movement.distance>15|buff.out_of_range.up
    if S.FelBlade:IsCastable() and (not Target:IsInRange(5) or Target:IsInRange(15)) then
      if AR.Cast(S.FelBlade) then return "Cast FelBlade"; end
    end
